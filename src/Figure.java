...
public class Figure {
    ...
    public void fileInput(BufferedReader input) throws IOException {
        ...
        if (figure.size() != 12) {
            throw new IOException("File must contain 12 lines and four numbers each line");
        } else {
            input.close();
        ...
        if (this.checkFigure(figure, n)) {
            if (n == 1) {
                figure.forEach((square) -> {
                    System.out.println(square.getTopLeft() + " " + square.getTopRight() + " " + square.getBottomLeft() + " " + square.getBottomRight() + " ");
                });
                System.out.println();
            } else {
                for(int i = 0; i < n; ++i) {
    ...
    private boolean checkFigure(ArrayList<Square> figure, int n) {
        int correctValue = 10;
        return (n != 1 || figure.get(0).getTopRight() + figure.get(1).getTopLeft() <= correctValue) &&
